...
func TestStripReleaseGroupFromTitle(t *testing.T) {
	cases := []struct{ in, want string }{
		{"   ", ""},
		{"Movie.2023.1080p.BluRay.x264-FRDS", "Movie.2023.1080p.BluRay.x264"},
		{"Show.S01E01.1080p.WEB-DL.AAC-HDS", "Show.S01E01.1080p.WEB-DL.AAC"},
		{"名.2020.2160p.HDR.HEVC-FRDS", "名.2020.2160p.HDR.HEVC"},
		{"Movie 2023 1080p BluRay x264-NF", "Movie 2023 1080p BluRay x264"},
	...
	for _, c := range cases {
		if got := StripReleaseGroupFromTitle(c.in); got != c.want {
			t.Errorf("StripReleaseGroupFromTitle(%s) = %q, want %q", c.in, got, c.want)
		}
	}
}

func TestStripReleaseGroupFromStem(t *testing.T) {
	cases := []struct{ in, group, want string }{
		{"   ", "", ""},
		{"Movie.2023.1080p.BluRay.x264-GROUP", "", "Movie.2023.1080p.BluRay.x264"},
		{"Movie.2023.1080p.BluRay.x264-FRDS", "", "Movie.2023.1080p.BluRay.x264"},
		{"Show.S01E01.1080p.WEB-DL.AAC-HDS", "", "Show.S01E01.1080p.WEB-DL.AAC"},
		{"名.2020.2160p.HDR.HEVC-FRDS", "", "名.2020.2160p.HDR.HEVC"},
		{"Movie 2023 1080p BluRay x264-GROUP", "", "Movie 2023 1080p BluRay x264"},
		{"Movie 2023 1080p BluRay x264-NF", "", "Movie 2023 1080p BluRay x264"},
		{"Movie.2023-GROUP.mkv", "", "Movie.2023-GROUP"},
		{"The.Matrix.1999.1080p.BluRay.x264", "", "The.Matrix.1999.1080p.BluRay"},
		{"Movie.2023.1080p", "", "Movie.2023"},
		{"Movie.Edition.Extended.2023", "", "Movie.Edition.Extended"},
		{"  Movie 2023  ", "", "Movie 2023"},
		{"Movie-2023-1080p-Team", "", "Movie-2023-1080p"},
		{"abcdefgh-GROUP", "", "abcdefgh"},
		{"Movie_2023_1080p_x265-GROUP", "", "Movie_2023_1080p_x265"},
		{"Love.Death.and.Robots.S01E01-GROUP", "", "Love.Death.and.Robots.S01E01"},
		{"Spider-Man.2002.1080p-GROUP", "", "Spider-Man.2002.1080p"},
		{"流浪地球2.The.Wandering.Earth.II.2023-GROUP", "", "流浪地球2.The.Wandering.Earth.II.2023"},
		{"", "", ""},
		{"-", "", "-"},
		{".", "", "."},
		{"..", "", ".."},
		...
		{"ab", "", "ab"},
		{"   ", "GROUP", ""},
		{"Movie.2023.1080p.BluRay.x264-GROUP", "GROUP", "Movie.2023.1080p.BluRay.x264"},
		{"Movie.2023.1080p.BluRay.x264-FRDS", "GROUP", "Movie.2023.1080p.BluRay.x264"},
		{"Title-GROUP", "GROUP", "Title"},
		{"Show.S01E01.1080p.WEB-DL.AAC-HDS", "GROUP", "Show.S01E01.1080p.WEB-DL.AAC"},
		{"名.2020.2160p.HDR.HEVC-FRDS", "GROUP", "名.2020.2160p.HDR.HEVC"},
		{"Movie 2023 1080p BluRay x264-GROUP", "GROUP", "Movie 2023 1080p BluRay x264"},
		{"Movie 2023 1080p BluRay x264-NF", "GROUP", "Movie 2023 1080p BluRay x264"},
		{"Movie.2023-GROUP.mkv", "GROUP", "Movie.2023-GROUP"},
		{"Title _GROUP", "GROUP", "Title"},
		{"Title.GROUP", "GROUP", "Title"},
		{"The.Matrix.1999.1080p.BluRay.x264", "GROUP", "The.Matrix.1999.1080p.BluRay"},
		{"Movie.2023.1080p", "GROUP", "Movie.2023"},
		{"Movie.Edition.Extended.2023", "GROUP", "Movie.Edition.Extended"},
		{"  Movie 2023  ", "GROUP", "Movie 2023"},
		{"Movie-2023-1080p-Team", "GROUP", "Movie-2023-1080p"},
		{"A-GROUP", "GROUP", "A"},
		{"abcdefgh-GROUP", "GROUP", "abcdefgh"},
		{"abcdefg-GROUP", "GROUP", "abcdefg"},
		{"Movie_2023_1080p_x265-GROUP", "GROUP", "Movie_2023_1080p_x265"},
		{"Love.Death.and.Robots.S01E01-GROUP", "GROUP", "Love.Death.and.Robots.S01E01"},
		{"Spider-Man.2002.1080p-GROUP", "GROUP", "Spider-Man.2002.1080p"},
		{"流浪地球2.The.Wandering.Earth.II.2023-GROUP", "GROUP", "流浪地球2.The.Wandering.Earth.II.2023"},
		{"", "GROUP", ""},
		{"-", "GROUP", "-"},
		{".", "GROUP", "."},
		{"..", "GROUP", ".."},
	...
	for _, c := range cases {
		if got := StripReleaseGroupFromStem(c.in, ParsedMedia{ReleaseGroup: c.group}); got != c.want {
			t.Errorf("StripReleaseGroupFromStem(%s, group=%q) = %q, want %q", c.in, c.group, got, c.want)
		}
	}
}

func TestStripReleaseGroupStripsLeadingSeparators(t *testing.T) {
	cases := []struct{ in, want string }{
		{"-FRDS-HDS", "FRDS"},
		{"-Movie FRDS", "Movie"},
		{"_Movie HDS", "Movie"},
	...
	for _, c := range cases {
		if got := StripReleaseGroupFromTitle(c.in); got != c.want {
			t.Errorf("StripReleaseGroupFromTitle(%q) = %q, want %q", c.in, got, c.want)
		}
		if got := StripReleaseGroupFromStem(c.in, ParsedMedia{}); got != c.want {
			t.Errorf("StripReleaseGroupFromStem(%q) = %q, want %q", c.in, got, c.want)
		}
	}
}
...
func TestNormalizeVideoCodec(t *testing.T) {
	...
	for _, c := range cases {
		if got := normalizeVideoCodec(c.in); got != c.want {
			t.Errorf("normalizeVideoCodec(%s) = %q, want %q", c.in, got, c.want)
		}
	}
}

func TestAudioCodecRank(t *testing.T) {
	cases := []struct {
		in   string
		want int
	}{
	...
	for _, c := range cases {
		if got := audioCodecRank(c.in); got != c.want {
			t.Errorf("audioCodecRank(%s) = %d, want %d", c.in, got, c.want)
		}
	}
}
